import Navbar from '../Navbar'

export default function Acerca() {
  return (
    <main style={{ fontFamily:'sans-serif', background:'#f9f9f9', minHeight:'100vh' }}>
      <Navbar />
      <div style={{ maxWidth:'800px', margin:'0 auto', padding:'24px 16px' }}>
        <div style={{ textAlign:'center', marginBottom:'40px' }}>
          <h1 style={{ fontSize:'28px', fontWeight:'500', marginBottom:'12px' }}>
            puja<span style={{ color:'#1D9E75' }}>.pe</span>
          </h1>
          <p style={{ fontSize:'16px', color:'#555', lineHeight:'1.8', maxWidth:'600px', margin:'0 auto' }}>
            El marketplace de remates online para el mercado peruano.
          </p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:'16px', marginBottom:'40px' }}>
          {[
            ['🎯 Nuestra misión', 'Democratizar el acceso a remates y subastas en el Perú, conectando compradores y vendedores de manera transparente y segura.'],
            ['👁 Nuestra visión', 'Ser la plataforma líder de remates online en Latinoamérica, reconocida por su confiabilidad y variedad de productos.'],
            ['💎 Nuestros valores', 'Transparencia, confianza y comunidad son los pilares de Puja.pe. Cada transacción es una oportunidad de conectar personas.'],
            ['🚀 Nuestra historia', 'Nacimos en 2026 con la idea de crear un espacio donde los peruanos puedan rematar y comprar artículos de forma simple y segura.'],
          ].map(([titulo, contenido]) => (
            <div key={titulo} style={{ background:'#fff', border:'1px solid #eee', borderRadius:'12px', padding:'20px' }}>
              <h2 style={{ fontSize:'15px', fontWeight:'500', marginBottom:'8px' }}>{titulo}</h2>
              <p style={{ fontSize:'14px', color:'#555', lineHeight:'1.7' }}>{contenido}</p>
            </div>
          ))}
        </div>
        <div style={{ background:'#E1F5EE', border:'1px solid #9FE1CB', borderRadius:'12px', padding:'24px', textAlign:'center' }}>
          <h2 style={{ fontSize:'18px', fontWeight:'500', marginBottom:'10px', color:'#085041' }}>¿Quieres unirte a Puja.pe?</h2>
          <p style={{ fontSize:'14px', color:'#0F6E56', marginBottom:'16px' }}>Regístrate gratis y empieza a comprar o vender hoy mismo.</p>
          <a href='/login' style={{ padding:'11px 28px', background:'#1D9E75', color:'white', borderRadius:'8px', textDecoration:'none', fontSize:'14px', fontWeight:'500' }}>
            Crear cuenta gratis
          </a>
        </div>
      </div>
    </main>
  )
}