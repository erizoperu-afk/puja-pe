          ...
          <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
            {fotosUrl.map((url, i) => (
              <div key={i} style={{ position:'relative' }}>
                ...
                <button onClick={() => { setFotos(fotos.filter((_, j) => j !== i)); setFotosUrl(fotosUrl.filter((_, j) => j !== i)) }} style={{ position:'absolute', top:'-8px', right:'-8px', width:'22px', height:'22px', borderRadius:'50%', background:'#E24B4A', color:'white', border:'none', cursor:'pointer', fontSize:'12px' }}>x</button>
              </div>
            ))}
            {fotos.length < 3 && (
