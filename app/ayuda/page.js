...
const FAQS = [
  ...
  },
  {
    ...
    preguntas: [
      ...
      { p: '¿Qué son los créditos?', r: 'Cada publicación consume 1 crédito. Durante el período BETA, los créditos son gratuitos e ilimitados. Próximamente habrá paquetes de créditos disponibles.' },
  ...
  return (
    <main style={{ fontFamily:'sans-serif', background:'#f9f9f9', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:'800px', margin:'0 auto', padding:'48px 24px' }}>
        <h1 style={{ fontSize:'28px', fontWeight:'500', marginBottom:'8px' }}>Centro de ayuda</h1>
        <p style={{ fontSize:'14px', color:'#666', marginBottom:'40px' }}>Encuentra respuestas a las preguntas más frecuentes.</p>

        {FAQS.map((seccion, si) => (
          <div key={si} style={{ marginBottom:'32px' }}>
            <h2 style={{ fontSize:'18px', fontWeight:'500', marginBottom:'16px' }}>{seccion.categoria}</h2>
              ...
              return (
                <div key={key} style={{ background:'#fff', border:'1px solid #eee', borderRadius:'10px', marginBottom:'8px', overflow:'hidden' }}>
                  <button onClick={() => setAbierto(isOpen ? null : key)}
                    ...
                    <span style={{ fontSize:'18px', color:'#1D9E75', flexShrink:0, marginLeft:'12px' }}>{isOpen ? '−' : '+'}</span>
                  </button>
                  {isOpen && (
                    <div style={{ padding:'0 16px 16px', fontSize:'14px', color:'#555', lineHeight:'1.7' }}>
                      {faq.r}
                    </div>
                  )}
                </div>
        ...
        <div style={{ background:'#E1F5EE', border:'1px solid #9FE1CB', borderRadius:'12px', padding:'24px', textAlign:'center', marginTop:'16px' }}>
          <p style={{ fontSize:'15px', color:'#085041', marginBottom:'12px' }}>¿No encontraste lo que buscabas?</p>
