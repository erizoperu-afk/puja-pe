    ...
    <main style={{ fontFamily: 'sans-serif' }}>
      ...
      <section style={{ background:'#f9f9f9', padding:'48px 24px', textAlign:'center', borderBottom:'1px solid #eee' }}>
        ...
        <div style={{ display:'flex', gap:'10px', maxWidth:'480px', margin:'0 auto' }}>
          <input
            type="text"
            placeholder="Busca laptops, celulares, autos..."
      ...
      <section style={{ padding:'24px' }}>
        <h2 style={{ fontSize:'16px', fontWeight:'500', marginBottom:'16px' }}>Remates activos</h2>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:'16px' }}>
          {['Laptop Dell XPS 13', 'iPhone 15 Pro', 'Bicicleta Trek'].map((nombre, i) => (
            <div key={i} style={{ background:'#fff', border:'1px solid #eee', borderRadius:'12px', overflow:'hidden' }}>
              ...
              <div style={{ padding:'12px' }}>
                <p style={{ fontWeight:'500', fontSize:'14px', marginBottom:'4px' }}>{nombre}</p>
                <p style={{ fontSize:'16px', fontWeight:'500' }}>S/ {[1850, 2400, 680][i].toLocaleString()}</p>
                <div style={{ display:'flex', justifyContent:'space-between', marginTop:'6px' }}>
                  <span style={{ fontSize:'11px', color:'#999' }}>{[23,41,8][i]} pujas</span>
                  <span style={{ fontSize:'11px', background:'#FCEBEB', color:'#A32D2D', padding:'2px 8px', borderRadius:'20px' }}>{['1h 12m','2h 05m','3h 48m'][i]}</span>
                </div>
              </div>
            </div>
          ))}
